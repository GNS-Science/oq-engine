...
class HassaniAtkinson2020SInter(GMPE):
    ...
    def get_mean_and_stddevs(self, sites, rup, dists, imt, stddev_types):
        ...
        mbreak_PGA = m_b + self._get_delta_mb(PGA())
        dsigma = self._dsigma(rup.hypo_depth)

        fm = self._fm_ha18(C, mbreak, rup.mag)
        fm_pga = self._fm_ha18(C_PGA, mbreak_PGA, rup.mag)
    ...
    def _fm_ha18(self, C, Mbreak, mag):
        """
        Magnitude factor.
        """
        # Mbreak = C['mh']
        if mag <= Mbreak:
            return C['e0'] + C['e1'] * (mag - Mbreak) \
                   + C['e2'] * (mag - Mbreak) ** 2
        return C['e0'] + C['e3'] * (mag - Mbreak)
